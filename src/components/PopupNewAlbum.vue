  ...
  <b-modal :active.sync="isOpen" has-modal-card :onCancel="handleCloseModal">
    <div class="modal-card">
      <header class="modal-card-head">
        <p class="modal-card-title">Add New Album</p>
      </header>
      ...
      <footer class="modal-card-foot">
        ...
        <button class="button is-primary" :disabled="disabled" @click="submit">Save</button>
      </footer>
    </div>
  </b-modal>
</template>
<script>
  ...
  export default {
    ...
    computed: {
      ...
      disabled(){
        return (!this.imageBase64 || this.imageBase64.trim().length < 1 || this.title.trim().length < 1)
      }
    },
    watch: {
      file: function() {
    ...
    methods: {
      ...
      submit: function() {
        const { title, imageBase64 } = this;
        this.addNewAlbum({ title, imageUrl: imageBase64 });
